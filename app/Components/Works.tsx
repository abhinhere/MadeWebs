...
export default function Works() {
  return (
    <section id="works" className="relative z-10 w-full px-4 md:px-14 py-10 md:py-12 flex flex-col justify-center items-start gap-4 md:gap-8 overflow-hidden">

      <div className="w-full flex flex-col items-start gap-4">
        <h2 className="text-[clamp(3rem,5vw,5rem)] font-melody tracking-tighter text-white leading-none">
          Our Works
        </h2>
      ...
      <div className="w-full mt-4 md:mt-8 overflow-x-auto md:overflow-x-visible pb-4 md:pb-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        <div className="flex flex-row md:grid md:grid-cols-4 gap-6 md:gap-10 w-[max-content] md:w-full pr-8 md:pr-0">
          {worksData.map((work, idx) => (
            <a key={idx} href={`https://${work.link}`} target="_blank" rel="noopener noreferrer" className="relative w-[75vw] md:w-full aspect-[4/3] rounded-2xl overflow-hidden shrink-0 group cursor-pointer" >
              <Image
                src={work.image}
                alt={work.title}
                fill
